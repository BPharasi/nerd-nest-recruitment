...
// Example: bookmarked job IDs (replace with real logic or localStorage)
// Only use IDs that exist in the jobs array from jobs page
const bookmarkedJobIds = Array.isArray(jobs) ? jobs.map(job => job.id).slice(0, 2) : []; // Example: first two jobs from jobs list
...
const BookmarkedJobsPage: NextPage = () => {
  const { data: session } = useSession();
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  // Only show jobs that exist in the jobs list and are bookmarked
  const bookmarkedJobs = Array.isArray(jobs)
    ? jobs.filter(job => job && typeof job.id === 'number' && bookmarkedJobIds.includes(job.id))
    : [];

  return (
    <div style={{ height: '100vh', width: '100vw', overflow: 'hidden', background: 'linear-gradient(to right, #283593, #5C6BC0)' }}>
      <Head>
        <title>Bookmarked Jobs - UFS Recruitment</title>
      </Head>
      {/* Header */}
      <header style={{
        height: '75px',
        ...
        <div className="max-w-7xl mx-auto" style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div className="mb-8 flex items-center">
            ...
            <h1 className="text-2xl font-bold text-white flex items-center gap-2">
              <FaBookmark className="text-yellow-400" /> Bookmarked Jobs
            </h1>
          </div>
                ...
                >
                  ...
                  <div className="p-6 flex-grow flex flex-col">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      {'duration' in job && (typeof job.duration === 'string' || typeof job.duration === 'number') && (
